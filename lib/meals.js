...
const appendMeal = (meal) => {
  $('.meals').append(`<table id=${meal.id} class='meal-foods-table'>
    ...
    .append(`<tr><td class='total'>Total Calories: </td><td class='meal-calories'>${mealCalorieTotal(meal)}</tr>`)
    .append(`<tr class='remaining-cal'><td>Remaining Calories: </td>${remCal(meal)}</tr>`)
}
...
const remCal = (meal) => {
  ...
  else if(meal.name === 'breakfast') {calories = 400 - mealCalorieTotal(meal)}
  else if(meal.name === 'lunch') {calories = 600 - mealCalorieTotal(meal)}
  else if(meal.name === 'dinner') {calories = 800 - mealCalorieTotal(meal)}
  if(calories >= 0){return `<td class='green-text'> ${calories} </td>`}
  else if(calories < 0){return `<td class='red-text'> ${calories} </td>`}
}
